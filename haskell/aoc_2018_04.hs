  ...
  obsParser = do
    ...
    A.choice
      ...
      , do
        ...
        if hour == 0
          then return
            (ObsRecord month day (ObsInPeriodGuardChange guard minute))
...
keyOfMaxValue m = case M.foldlWithKey' f Nothing m of
 ...
 where
  ...
  f (Just (maxG, maxT)) g t | t > maxT  = Just (g, t)
...
maxMinute = keyOfMaxValue . countMinutes
 where
  ...
  addInterval m (Minute t1, Minute t2) =
    foldl' addMinute m [ Minute t | t <- [t1 .. t2 - 1] ]
...
maxPair triplets = keyOfMaxValue countMinutes
 where
  ...
  addInterval m (g, Minute t1, Minute t2) =
    foldl' (addMinute g) m [ Minute t | t <- [t1 .. t2 - 1] ]
  addMinute
...
answer debug input = do
  ...
  return
    ( show (sleepiestGuard * sleepiestMinute)
    , show (sleepiestPairGuard * sleepiestPairMinute)
    )
 where
...
main = do
  ...
  debug    <- fmap ("-d" `elem`) getArgs
  (a, b)   <- answer debug contents
  putStrLn a
  putStrLn b
